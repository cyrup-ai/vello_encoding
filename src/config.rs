...
// GPU workgroup size constants - obtained from vello_shaders analysis
// These values are standard across GPU compute shaders for optimal utilization
...
pub struct ConfigUniform {
    ...
    /// Size of blend spill buffer (in tile blocks of TILE_WIDTH * TILE_HEIGHT pixels).
    /// Each tile block represents 16x16=256 pixels for optimal GPU cache utilization.
    pub blend_size_tiles: u32,
    /// Size of per-tile command list buffer allocation (in `u32`s).
    pub ptcl_size: u32,
}

impl ConfigUniform {
    /// Converts blend buffer size from tile blocks to total pixels.
    /// Each tile block represents TILE_WIDTH * TILE_HEIGHT pixels for optimal GPU cache utilization.
    pub fn blend_size_pixels(&self) -> u32 {
        self.blend_size_tiles * TILE_WIDTH * TILE_HEIGHT
    }
}
    ...
    pub fn new(layout: &Layout, width: u32, height: u32, base_color: &peniko::Color) -> Self {
        ...
        Self {
            gpu: ConfigUniform {
                ...
                blend_size_tiles: buffer_sizes.blend_spill.len() / (TILE_WIDTH * TILE_HEIGHT),
