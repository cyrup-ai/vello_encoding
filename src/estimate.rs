...
pub struct BumpEstimator {
    // GPU workload estimation for proper buffer allocation
    binning_buffer_size: u32, // Binning stage buffer estimate
    ptcl_buffer_size: u32,    // Path-tile-command-list buffer estimate
    tile_buffer_size: u32,    // Tile rendering buffer estimate
...
impl BumpEstimator {
    ...
    pub fn append(&mut self, other: &Self, transform: Option<&Transform>) {
        ...
        // Scale GPU buffer estimates using transform_scale pattern
        self.binning_buffer_size += (other.binning_buffer_size as f64 * scale).ceil() as u32;
        self.ptcl_buffer_size += (other.ptcl_buffer_size as f64 * scale).ceil() as u32;
        self.tile_buffer_size += (other.tile_buffer_size as f64 * scale).ceil() as u32;
    }
    ...
    pub fn count_path(
        ...
        let Some(style) = stroke else {
            ...
            // Calculate GPU buffer estimates for fill paths
            let total_elements = lineto_lines + fill_close_lines + curve_count;
            self.binning_buffer_size += (total_elements as f64 * scale).ceil() as u32;
            self.ptcl_buffer_size += (segments as f64 * 1.5 * scale).ceil() as u32; // Path-tile commands scale with segments
            self.tile_buffer_size += (segments as f64 * 0.75 * scale).ceil() as u32; // Tile rendering scales with coverage
    ...
    pub fn tally(&self, transform: Option<&Transform>) -> BumpAllocatorMemory {
        ...
        // Scale all GPU buffer estimates with final transform
        let scaled_binning =
            ((self.binning_buffer_size as f64 * scale).ceil() as u32).max(n_segments);
        let scaled_ptcl = (self.ptcl_buffer_size as f64 * scale).ceil() as u32;
        let scaled_tile = (self.tile_buffer_size as f64 * scale).ceil() as u32;

        let bump = BumpAllocators {
            failed: 0,
            // Use calculated GPU buffer estimates instead of hardcoded assumptions
            binning: scaled_binning,
            ptcl: scaled_ptcl,
            tile: scaled_tile,
            blend: 0,
...
fn estimate_arc_lines(scaled_stroke_width: f64) -> (u32, f64) {
    ...
    // flatten.wgsl - tolerance definitions shared with GPU pipelines for consistency
    // Using configurable tolerance parameters that align with GPU pipeline config
    // This ensures synchronization between CPU estimation and GPU execution
...
// Optimized 32-bit transform for better performance in GPU estimation
fn transform(t: &Transform, v: Vec2) -> Vec2 {
    // Use f32 precision for GPU estimation contexts where full f64 precision isn't needed
    let x32 = v.x as f32;
    let y32 = v.y as f32;
    let m0 = t.matrix[0] as f32;
    let m1 = t.matrix[1] as f32;
    let m2 = t.matrix[2] as f32;
    let m3 = t.matrix[3] as f32;

    Vec2::new((m0 * x32 + m2 * y32) as f64, (m1 * x32 + m3 * y32) as f64)
}
...
fn count_segments_for_line_length(scaled_width: f64) -> u32 {
    // scale the tile count by sqrt(2) to allow optimal slack for diagonal lines.
    // Using sqrt(2) provides mathematically accurate diagonal line coverage
    // Factor of 2 would be excessive, sqrt(2) ≈ 1.414 is optimal for 45-degree diagonals
